...
BEGIN
  ...
  -- Salle double-réservation (salle_id ou texte salle)
  RETURN QUERY
  ...
  -- Volume horaire dépassé / non atteint
  RETURN QUERY
  WITH actual AS (
  ...
  SELECT 'volume'::text,
         CASE WHEN actual.h > cm.volume_horaire_hebdo THEN 'critique' ELSE 'avertissement' END,
         NULL::smallint, NULL::time, NULL::time,
