...
BEGIN
  ...
  -- 1) OVERRIDE explicite : REMPLACER l'echeancier en preservant les tranches deja encaissees
  IF v_override IS NOT NULL THEN
    ...
    v_montant_unit := ROUND(v_total / v_nb);

    -- (a) Supprimer TOUTES les tranches sans paiement (peu importe le frais_id)
    DELETE FROM tranches
     WHERE eleve_id = _eleve_id
       AND NOT EXISTS (SELECT 1 FROM paiements p WHERE p.tranche_id = tranches.id);

    -- (b) Renumeroter les tranches deja encaissees (paiements existants) et les migrer
    --     vers la nouvelle grille. Les IDs des tranches restent inchanges, donc les recus
    --     et paiements existants continuent de pointer sur la meme ligne.
    --     Deux passes pour eviter les collisions sur l'unique (eleve_id, frais_id, numero).
    WITH ranked AS (
  ...
  -- 2) Mode automatique historique (base sur la classe)
