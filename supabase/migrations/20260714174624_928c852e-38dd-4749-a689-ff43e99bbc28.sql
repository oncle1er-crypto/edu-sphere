...
BEGIN
  ...
  IF v_override IS NOT NULL THEN
    ...
    -- Passe finale : si _force_recalc = true, on met aussi a jour le montant des tranches deja encaissees.
    -- Sinon on ne touche qu'au libelle/echeance des tranches deja payees, et on ne recalcule le montant que pour les tranches non payees.
    UPDATE tranches t
       SET numero = -t.numero,
           label = CASE WHEN -t.numero = 1 THEN '1ere tranche' ELSE 'Tranche ' || (-t.numero) END,
           montant = CASE
                       WHEN _force_recalc OR t.paye = 0 THEN v_montant_unit
                       ELSE t.montant
                     END,
           echeance = v_annee_debut + ((v_annee_fin - v_annee_debut) * (-t.numero) / v_nb),
           statut = CASE
                      WHEN t.paye <= 0 THEN 'due'::statut_tranche
                      WHEN t.paye >= (CASE WHEN _force_recalc OR t.paye = 0 THEN v_montant_unit ELSE t.montant END) THEN 'soldee'::statut_tranche
