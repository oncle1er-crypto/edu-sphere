  ...
  return useQuery<BilanComptable>({
    ...
    queryFn: async () => {
      ...
      for (const t of (tranches ?? []) as any[]) {
        ...
        trancheIds.add(t.id);
        // En revanche, le "total dû" (attendu, prospectif) exclut les élèves
        // sortis/exclus/transférés — cohérent avec useFinanceData.ts
        // (Finances > Frais attendus), sinon le Bilan comptable affiche un
        // total dû différent de celui de Finances pour le même exercice.
