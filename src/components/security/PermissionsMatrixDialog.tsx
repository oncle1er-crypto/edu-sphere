...
export function PermissionsMatrixDialog({ open, onOpenChange, userId, userName }: Props) {
  const { modules, perms, loading, saving, isDefault, toggle, setAllForModule, setActionForModules, applyPreset, save } = useUserPermissions(userId);
        ...
        <div className="flex flex-col gap-3 py-2">
          ...
          {!loading && isDefault && (
            <div className="rounded-md border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 px-3 py-2 text-xs text-amber-900 dark:text-amber-200">
              Aucune permission individuelle n'est encore enregistrée pour cet utilisateur. Les cases pré-cochées ci-dessous correspondent à ce que <strong>son rôle</strong> lui accorde (ou aux valeurs par défaut recommandées). Ajustez-les puis cliquez sur <strong>Enregistrer</strong> pour créer une surcharge individuelle.
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary">
              {selected.size > 0
                ...
                : `Cible : ${visibleKeys.length} module${visibleKeys.length > 1 ? "s" : ""} affiché${visibleKeys.length > 1 ? "s" : ""}`}
            </Badge>

            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" variant="outline">
                  Appliquer un preset <ChevronDown className="ml-1 h-3 w-3" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56">
                <DropdownMenuLabel>
                  {selected.size > 0 ? "Aux lignes sélectionnées" : "À toutes les lignes affichées"}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={() => applyPreset("full", targetKeys())}>
                  <Shield className="mr-2 h-4 w-4 text-primary" /> Plein accès
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => applyPreset("readonly", targetKeys())}>
                  <Eye className="mr-2 h-4 w-4" /> Lecture seule
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => applyPreset("none", targetKeys())}>
                  <ShieldOff className="mr-2 h-4 w-4 text-muted-foreground" /> Aucun accès
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" variant="outline">
                  Action en masse <ChevronDown className="ml-1 h-3 w-3" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-64">
                <DropdownMenuLabel>
                  {selected.size > 0 ? "Sur les lignes sélectionnées" : "Sur les lignes affichées"}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                {ACTIONS.map(a => (
                  <div key={a.key}>
                    <DropdownMenuItem onClick={() => setActionForModules(a.key, true, targetKeys())}>
                      <a.icon className="mr-2 h-4 w-4 text-primary" /> Activer « {a.label} »
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setActionForModules(a.key, false, targetKeys())}>
                      <a.icon className="mr-2 h-4 w-4 text-muted-foreground" /> Désactiver « {a.label} »
                    </DropdownMenuItem>
                  </div>
                ))}
              </DropdownMenuContent>
            ...
            {selected.size > 0 && (
              <Button size="sm" variant="ghost" onClick={() => setSelected(new Set())}>
                <X className="mr-1 h-3 w-3" /> Effacer la sélection
              </Button>
            )}
          </div>
        </div>
            ...
            <Table>
                    ...
                    return (
                      <TableHead key={a.key} className="text-center">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <button
                            onClick={() => setActionForModules(a.key, !allOn, keys)}
                            className="text-[10px] text-primary hover:underline"
                            title={selected.size > 0 ? "Basculer sur la sélection" : "Basculer sur les lignes affichées"}
                          >
              ...
              <TableBody>
                {filteredModules.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={ACTIONS.length + 3} className="text-center text-muted-foreground py-8">
                ...
                {filteredModules.map(m => {
                  ...
                  return (
                    <TableRow key={m.key} data-state={isSelected ? "selected" : undefined}>
                      <TableCell>
                        <Checkbox checked={isSelected} onCheckedChange={() => toggleRow(m.key)} />
                      </TableCell>
