...
export default function CanteenBilling() {
  ...
  const fetchData = async () => {
    if (!ecoleId) return;
    const { data } = await supabase.from("factures").select("id, numero, libelle, montant, montant_paye, statut, date_echeance, eleves(nom, prenom)").eq("ecole_id", ecoleId).eq("categorie", "cantine").order("created_at", { ascending: false });
  ...
  return (
    <SettingsSection title="Facturation cantine" description="Génération et suivi des factures de restauration." icon={<Receipt className="h-5 w-5" />} hideSave>
      ...
      <div className="border rounded-lg overflow-x-auto">
        <Table>
          ...
          <TableBody>
            ...
            {rows.length === 0 && (
              <TableRow><TableCell colSpan={6} className="text-center text-sm text-muted-foreground py-8">Aucune facture cantine.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
