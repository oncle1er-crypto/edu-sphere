...
import {
  LayoutDashboard, Library, BookOpen, Users, Repeat, AlertCircle,
  Search, FileText, Settings2, BarChart3, Tag, Archive,
} from "lucide-react";
...
const sections = [
  ...
  { to: "catalogue", label: "Catalogue", icon: BookOpen, group: "Vue d'ensemble" },

  { to: "recherche", label: "Recherche avancée", icon: Search, group: "Catalogue" },
  { to: "categories", label: "Catégories & tags", icon: Tag, group: "Catalogue" },
  { to: "acquisitions", label: "Acquisitions", icon: Archive, group: "Catalogue" },

  { to: "emprunts", label: "Emprunts en cours", icon: Repeat, group: "Circulation" },
  { to: "retards", label: "Retards & relances", icon: AlertCircle, group: "Circulation" },
  { to: "lecteurs", label: "Lecteurs", icon: Users, group: "Circulation" },
...
export default function LibraryLayout() {
  ...
  if (location.pathname === "/bibliotheque" || location.pathname === "/bibliotheque/") {
    return <Navigate to="/bibliotheque/tableau" replace />;
  }
    ...
    <div className="space-y-6">
      <div className="sticky top-[7.25rem] z-20 rounded-2xl bg-card/90 backdrop-blur-md border border-border/60 shadow-[var(--shadow-card)] px-4 py-3">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Catalogue, emprunts, retards et statistiques de lecture.
        </p>
      </div>
